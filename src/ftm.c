...
typedef struct {
    ...
    // Indexed as pattern_map[channel * FTM_MAX_PATTERN + pattern].
    FtNote** pattern_map;
...
static FamResult blob_write_bytes(Blob* blob, uint8_t* bytes, size_t count) {
    // Resize blob if needed
    if (blob->capacity < blob->size + count) {
...
static FamResult block_read_instruments(BufferReader* reader, uint32_t block_version, FamFtModule* module) {
    ...
    // Instrument indices are sparse within [0, FTM_MAX_INSTRUMENTS); the fixed
...
static FamResult block_read_sequences(BufferReader* reader, uint32_t block_version, FamFtModule* module) {
    ...
    if (block_version == 1) {
        ...
        if (block_version == 5) {
            ...
            // The file stores all FTM_MAX_SEQUENCES * 5 entries, so read them all
...
static FamResult block_read_dpcm_samples(BufferReader* reader, uint32_t block_version, FamFtModule* module) {
    ...
    // Sample indices are sparse within [0, FTM_MAX_DPCM_SAMPLES); the fixed
    // module->samples slot array is already zeroed.
    Blob sample_blob;
    FamResult err = blob_init(&sample_blob, 0x4000);
    if (err != FAM_SUCCESS) {
        return err;
    }

    // The blob holds each sample's raw bytes back-to-back with no padding; the
    // 64-byte alignment the NES requires is applied later, per bank, during
    // conversion. Each sample records its byte offset into the blob.
    uint32_t offset = 0;
    for (int i = 0; i < sample_count; i++) {
        ...
        // Copy the raw sample bytes straight into the blob (bounds-checked first,
        // since we read from the cursor position directly).
        if (sample->length > buffer_reader_remaining(reader)) {
            return FAM_ERROR_INVALID_FORMAT;
        }
        err = blob_write_bytes(&sample_blob, reader->pos, sample->length);
        if (err != FAM_SUCCESS) {
            return err;
        }
    ...
    printf("Sample blob size: %zu\n", sample_blob.size);
    module->sample_data = sample_blob.data;
